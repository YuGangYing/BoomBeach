...
namespace BoomBeach{
	...
	public class DataManager : SingleMonoBehaviour<DataManager>{
		...
		public Dictionary<string,Queue<BuildInfo>> buildObjectPrefabList = new Dictionary<string, Queue<BuildInfo>>();
		public Hashtable buildList;//当前场景建筑；
		public List<BuildInfo> buildArray;//所有可移动的建筑列表，不含树，石，船; 
		public Hashtable userRegionsList = new Hashtable();//用户已经已开启的岛屿;UserRegions；regions_id 为key

		public UserInfo userInfo = new UserInfo();
		public long userId;
		public Hashtable researchLevel = new Hashtable();//用户实研室中可升级tid等级;
		public List<BattleTrooperData> battleEnergyList = new List<BattleTrooperData>();//当前可以使用能量弹;
		public SceneStatus sceneStatus;  //当前场景状态;

		protected override void Awake(){
			buildArray = new List<BuildInfo> ();
		}

		public IslandType islandType = IslandType.playerbase;//岛类型;
		public GameObject sceneIsland;//当前的岛屿;
		public GameObject gameManager;//游戏逻辑

		public Vector3 dockPos = new Vector3(16f,-1f,-7f);
		public Vector3 gunboatPos = new Vector3(8f,-1f,-10f);

		public int maxTownHallLevel = 20;//最大主堡等级;
		public long lastLoadTime;/*最后加载时间,从服务器返回;*/
		public long time_difference = 0;/*服务器与客服端时差 =;*/
		public GridInfo[,] GridArray;
		public int GridTotal = 50; //常量，建筑区域大小;
		public int GridBuildCost = 0; //不可通行权重;
		public int GridEmptyCost = 1;  //可通行权重;
		public byte[,] mMatrix; //AStar寻路格子;
	    //public static List<Models.BuildingData> buildings;
	    //public List<BuildInfo> buildArray;  //所有可移动的建筑列表，不含树，石，船;  
		public int regions_id = 0;//当前用户所在地图;0:为主地图；n:副本地图;
		...
		public int LastSceneUserId = -1;//最后加载的场景用户id;
		public int LastSceneRegionsId = -1;//最后加载的d岛屿id;
		public List<ShopCate> ShopCates = new List<ShopCate> ();//底部板块数据;
		public IList<PopUI> popUIList; 

		public BuildInfo currentTrainBuildInfo;
		...
		public Dictionary<string,int[,]> island_grid_csv = new Dictionary<string,int[,]>();//岛的不可通行区配置;
		public bool IsSceneLoaded = false;
		public ArrayList LandCrafts;
		public Hashtable EnemyActivityList = new Hashtable();//世界地图中的被攻击列表数据;EnemyActivityItem
		public float SearchCostFactor = 0.3f;//搜索为攻击成本的倍率;
		//当前可以使用出战的军队;
		public List<BattleTrooperData> battleTrooperList = new List<BattleTrooperData>();
		public Dictionary<string,Projectiles> projectileData = new Dictionary<string, Projectiles> ();
		...
		//可更新岛屿的时间间隔;
		public int FindNewOpponentTime = 79200;//默认22小时，可更新一次(只能更新比自己级别高的或资源岛屿;
		public int overall_damage_level;//当前建筑物被摧毁级别(1,2,3);
		//全局变量初始化;
		public void Init()
		{
