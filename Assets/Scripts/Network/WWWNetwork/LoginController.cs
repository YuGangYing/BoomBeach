...
//		}
	...
	void Update()
	{
		if(Input.GetKeyDown(KeyCode.H)){
			WWWNetworkManagerOld.GetInstance.Login ("Tester","123456",LoginCallBack);
		}
	}

	void Init()
	{
		WWWNetworkManagerOld.GetInstance.GetBuildingInfos (GetBuildingInfoCallBack);
...
//		Debug.Log (DataCenter.Instance().userBuildingList.Count);
	...
	void LoginCallBack(WWW www){
		WWWNetworkManagerOld.GetInstance.cookies = www.ParseCookies ();
		Debug.Log (WWWNetworkManagerOld.GetInstance.cookies.Count);
...
//		DataCenter.Instance().userInfo = userInfo;
	...
	void Login()
	{
		WWWNetworkManagerOld.GetInstance.Login (userName.text,userPassword.text,LoginCallBack);
	}
