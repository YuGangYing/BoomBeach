...
namespace BoomBeach
{
	public class BuildManager : SingleMonoBehaviour<BuildManager>
	{
		...
		public void CreateBuilding(Network.BuildingModel buildingModel){
			MUnitCSVStructure unit = CSVManager.GetInstance.GetUnit (buildingModel.type,buildingModel.level);
			if (buildingModel.type == GameConstant.TID_BUILDING_LANDING_SHIP || buildingModel.type == GameConstant.TID_BUILDING_GUNSHIP) {
				
			} else {
				CreateBuilding (buildingModel,unit);
			}
		}
