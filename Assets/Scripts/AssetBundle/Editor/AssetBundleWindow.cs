...
//		ABOutputPath = Application.dataPath + "/Assetbundles/android/";
		...
		ABOutputPath = PathConstant.CLIENT_AB_PATH;
	...
	static void Reload ()
	{
		...
		if (!FileManager.DirectoryExists (ABOutputPath)) {
			FileManager.CreateDirectory (ABOutputPath);
		}
...
//		}
	...
	static void GetHash ()
	{
		...
		for (int i = 0; i < allAssetBundleEntitys.Count; i++) {
			...
			allAssetBundleEntitys [i].abHash = FileManager.GetFileHash (ABOutputPath + allAssetBundleEntitys [i].abName);
			if (allAssetBundleEntitys [i].abHash != null && allAssetBundleEntitys [i].abHash.Trim () != "") {
				allAssetBundleEntitys [i].realLength = new FileInfo (ABOutputPath + allAssetBundleEntitys [i].abName).Length;
	...
	int mBuildMode = 1;//1.全部ビルド（処理依頼を処理する）。2.部分ビルド（依赖関連を処理しない）
	void BuildABs ()
	{
		AssetDatabase.RemoveUnusedAssetBundleNames ();
		if (mBuildMode == 1) {
			...
			#elif UNITY_ANDROID
			BuildPipeline.BuildAssetBundles (ABOutputPath, BuildAssetBundleOptions.None, BuildTarget.Android);
			#else
			BuildPipeline.BuildAssetBundles (fullTmpOutputPath,BuildAssetBundleOptions.None);
			#endif
		}else if (mBuildMode == 2) {
			...
			#elif UNITY_ANDROID
			if(abList.Count>0){
				BuildPipeline.BuildAssetBundles (ABOutputPath,abList.ToArray(),BuildAssetBundleOptions.None, BuildTarget.Android);
			}
			#else
			BuildPipeline.BuildAssetBundles (fullTmpOutputPath,BuildAssetBundleOptions.None);
			#endif
...
//	}
	...
	void CopyToTemp ()
	{
		...
		for (int i = 0; i < allAssetBundleEntitys.Count; i++) {
			if (allAssetBundleEntitys [i].isSelected) {
				if(FileManager.Exists(ABOutputPath + allAssetBundleEntitys [i].abName))
				FileManager.CopyFile (ABOutputPath + allAssetBundleEntitys [i].abName, tempResourceAssetPath + allAssetBundleEntitys [i].abName);
			}
		}
		AssetDatabase.Refresh ();
	}
	...
	void CopyToResources ()
	{
		for (int i = 0; i < allAssetBundleEntitys.Count; i++) {
			if (allAssetBundleEntitys [i].isSelected) {
				FileManager.CopyFile (ABOutputPath + allAssetBundleEntitys [i].abName, fullResourceAssetPath + allAssetBundleEntitys [i].abName);
			}
		}
		AssetDatabase.Refresh ();
	}
	...
	void CopyToStream ()
	{
		for (int i = 0; i < allAssetBundleEntitys.Count; i++) {
			if (allAssetBundleEntitys [i].isSelected) {
				FileManager.CopyFile (ABOutputPath + allAssetBundleEntitys [i].abName, fullStreamPath + allAssetBundleEntitys [i].abName);
			}
		}
		AssetDatabase.Refresh ();
	}
...
//			if (abEntity.isSelected) {
			if (abEntity.abHash != null && abEntity.abHash.Trim () != "") {
				result.AppendLine (CreateLine (abEntity.abName, new FileInfo (ABOutputPath + abEntity.abName).Length, abEntity.abName == "csv.assetbundle" ? 1 : 0, abEntity.abHash).ToString ());
			}
//			}
		}
